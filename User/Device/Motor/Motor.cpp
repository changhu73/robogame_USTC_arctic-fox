...
void Class_Motor::Output()
{
    ...
    else if (Out > 0)  //8.2 jintai:猜测output>0时对应轴远离自己时看起来是逆时针
    {
    ...
    __HAL_TIM_SetCompare(&Driver_PWM_TIM, Driver_PWM_TIM_Channel_x, Math_Abs(Out));  //PWM输出
}
...
void Class_Motor_With_Hall_Encoder::Calculate_TIM_PeriodElapsedCallback()
{
    ...
    //计算角度
    Angle_Now += (float)delta / MOTOR_ENCODER_NUM_PER_SECOND;
    //计算速度
    Omega_Now = (float)delta / MOTOR_ENCODER_NUM_PER_SECOND / MOTOR_CALCULATE_PERIOD;
